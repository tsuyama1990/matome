...
We will build the `IngestionPipeline` service, orchestrating text parsing, intelligent chunking algorithms, and initial entity extraction and pre-tagging using the `LLMProtocol`. Crucially, we will implement a semantic chunking strategy that goes beyond simple character limits, utilizing cosine similarity between adjacent sentences to divide text at logical proposition boundaries, as specified in the PRD (FR-1.2). Furthermore, to support multi-modal ingestion (FR-1.1), the pipeline must orchestrate Vision-Language Models (VLMs) for parsing complex charts in PDFs. Lastly, every chunk must be pre-tagged with multidimensional metadata (Time, Logic, Polarity axes) via the LLM to enable high-performance Pivot operations in Cycle 06. We must also ensure that the generated embeddings strictly adhere to the dimensional constraints defined in the `SemanticChunk` Pydantic model. This cycle bridges the gap between raw data and the pure domain models, ensuring the resulting objects are pristine and ready for complex AI operations.
...
*   **`TextParserProtocol`**: A `typing.Protocol` for extracting raw string content from various document formats (PDF, Markdown, images). It must include an asynchronous method (as it might call VLMs): `async def parse(self, file_content: bytes, filename: str) -> str: ...`. For Cycle 03, while we will implement a basic `PlainTextParser` for simple tests, the protocol MUST be designed to accept image bytes and interface with `LLMProtocol` (specifically using the `multimodal_model` config) to extract text and context from charts and diagrams (FR-1.1).
...
*   **`IngestionPipeline`**: The core application service for this cycle.
    ...
    *   **`process_document(file_content: bytes, filename: str) -> list[SemanticChunk]`**: The main orchestration method.
        ...
        4.  **Extract Entities & Pre-tagging**: Concurrently (using `asyncio.gather`) call the `LLMProtocol` for each chunk with a complex prompt requesting entity extraction AND tagging (e.g., "Extract proper nouns. Also, categorize this text along the Time axis (Past/Present/Future) and Logic axis (Fact/Claim/Evidence). Return a JSON object").
        5.  **Instantiate Domain Models**: For each processed chunk, create a `ChunkMetadata` object (populated with extracted entities, the source filename, and the Time/Logic/Polarity tags derived from the LLM) and a `SemanticChunk` object.
...
5.  **Integrate LLM Extraction & Tagging**: Write an asynchronous helper method within `IngestionPipeline` that constructs a prompt, calls `llm_service.generate_text` (potentially enforcing JSON output), parses the resulting string to extract both a `list[str]` of entities and strings for the pre-calculated tags (Time Axis, Logic Axis).
...
Crucially, we will test the entity extraction and pre-tagging logic. We will configure the `DummyLLMService` to return a specific, recognizable JSON string when prompted (e.g., `{"entities": ["EntityA"], "time_axis": "Present"}`). We will assert that the `extracted_entities` list within the `ChunkMetadata` of the resulting chunks exactly matches the dummy output, AND that the `time_axis` field is correctly populated. This proves the pipeline is correctly orchestrating the complex LLM calls and mapping multi-dimensional metadata into the strict Pydantic structures.
