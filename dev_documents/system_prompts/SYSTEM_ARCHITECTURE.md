...
3.  **Strict Data Privacy & Security**: Enterprise users require absolute certainty that their data will not be used to train public models. The system must support Bring Your Own Key (BYOK) configurations, strong encryption for credentials, and a modular design that could support local, on-premise inference deployments if necessary.
...
3.  **`user_session.py` (New)**: Contains `LearningProgress` and `NodeInteraction` models to track which nodes a user has unlocked, their answers to questions, and their overall progress through the document tree.
...
*   **CYCLE03: Document Ingestion & Chunking Pipeline**
    *   **Focus**: Build the first half of the data processing pipeline. We will implement the semantic chunking logic and entity extraction, creating `SemanticChunk` objects from raw text.
    *   **Key Deliverables**: `src/application/ingestion.py` focusing on text parsing, chunking algorithms, and invoking the LLM for entity extraction.
