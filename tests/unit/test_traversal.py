...
def test_get_source_chunks(engine: InteractiveRaptorEngine, mock_store: MagicMock) -> None:
    ...
    root = SummaryNode(id="root", text="Root", level=2, children_indices=["c1", "c2"], metadata=NodeMetadata())
    child1 = SummaryNode(id="c1", text="C1", level=1, children_indices=[1, 2], metadata=NodeMetadata())
    ...
    def get_nodes_side_effect(ids: list[str | int]) -> dict[str | int, SummaryNode | Chunk | None]:
        results = {}
        for nid in ids:
            if nid == "c1": results["c1"] = child1
            elif nid == "c2": results["c2"] = child2
            elif nid == 1: results[1] = leaf1
            elif nid == 2: results[2] = leaf2
            elif nid == 3: results[3] = leaf3
        return results
