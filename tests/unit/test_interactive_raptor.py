...
def test_refine_node_not_found(interactive_engine: InteractiveRaptorEngine, mock_store: MagicMock) -> None:
    ...
    with pytest.raises(RefinementError, match="Refinement failed: Node .* not found"):
...
def test_refine_node_chunk_error(
    ...
    with pytest.raises(RefinementError, match="Unexpected error: Only SummaryNodes can be refined"):
...
def test_refine_node_invalid_instruction(
    ...
    with pytest.raises(RefinementError, match="Refinement failed: Instruction cannot be empty"):
    ...
    with pytest.raises(RefinementError, match="Refinement failed: Instruction exceeds maximum length"):
...
def test_refine_node_update_failure(
    ...
    with pytest.raises(RefinementError, match="Refinement failed: DB Write Failed"):
        interactive_engine.refine_node("s1", "Refine")
