...
class FileProcessingService:
    ...
    def _validate_filename(self, filename: str) -> str:
        ...
        # Prevent homograph attacks and IDN spoofing by normalizing to NFKC and strictly enforcing ASCII.
        normalized_filename = unicodedata.normalize("NFKC", filename)

        try:
            normalized_filename.encode("ascii")
        except UnicodeEncodeError as e:
            msg = "Filename must contain only ASCII characters to prevent homograph attacks."
            raise ValueError(msg) from e

        import re

        # Strictest validation: only allow ASCII alphanumeric, dash, and dot.
        if not re.match(r"^[a-zA-Z0-9.\-_]+$", normalized_filename):
