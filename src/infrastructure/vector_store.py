...
class PineconeVectorStore(VectorStoreProtocol):
    ...
    def _validate_collection_name(self, collection_name: str) -> None:
        import re
        # Pinecone namespace spec: lowercase alphanumeric and hyphens, up to 45 chars
        if not re.match(r"^[a-z0-9-]{1,45}$", collection_name):
            msg = "Invalid collection name. Pinecone requires lowercase alphanumeric and hyphens, max 45 chars."
    ...
    def upsert(self, collection_name: str, records: list[dict[str, Any]]) -> None:
        ...
        for r in records:
            ...
            # Validate metadata types natively supported by Pinecone rather than corrupting data via HTML escape
            ...
            for k, v in r.items():
                ...
                if isinstance(v, str | int | float | bool) or (isinstance(v, list) and all(isinstance(i, str) for i in v)):
                    ...
                    # Ignore invalid Pinecone metadata types silently to prevent injection/crashes
                    pass
