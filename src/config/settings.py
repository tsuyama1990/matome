...
class DatabaseConfig(BaseSettings):
    ...
    allow_local_connections: bool = Field(
        default=False, description="Allow local database connections for on-premise deployments."
    )
    ...
    def _validate_uri(self, uri: str) -> SecretStr:
        ...
        # Configurable SSRF prevention based on deployment mode
        if not self.allow_local_connections and parsed_uri.hostname in ("127.0.0.1", "localhost", "0.0.0.0", "::1"):  # noqa: S104
            msg = "Local database connections are not permitted by current security policy."
            raise ValueError(msg)

        # Strip userinfo before returning for enhanced security context
...
class ModelConfig(BaseSettings):
    """Configuration for LLMs via OpenRouter or Local Models."""

    openrouter_api_url: AnyHttpUrl = Field(
        description="The API endpoint for model inferences.",
    )
    ...
    allow_local_inference: bool = Field(
        default=False, description="Flag allowing HTTP and local hosts for on-premise execution."
    )
    ...
    def validate_https_url(cls, v: AnyHttpUrl, info: ValidationInfo) -> AnyHttpUrl:
        ...
        allow_local = info.data.get("allow_local_inference", False)

        if not allow_local and v.scheme != "https":
            msg = "API URL must use HTTPS unless local inference is enabled."
            raise ValueError(msg)
        ...
        if not allow_local and v.host in ("127.0.0.1", "localhost", "0.0.0.0", "::1"):  # noqa: S104
            msg = "Internal network hostnames are forbidden unless local inference is enabled."
            raise ValueError(msg)
