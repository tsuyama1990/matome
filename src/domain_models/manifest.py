...
class UserInteractionContext(BaseModel):
    ...
    node_id: str = Field(..., description="The node being interacted with", max_length=100, pattern=r'^[a-zA-Z0-9_-]+$')
    status: NodeStatus = Field(..., description="Current status before interaction")
    question_asked: str | None = Field(None, description="The AI generated question asked to the user", max_length=1000)
    user_answer: str | None = Field(None, description="The user's provided answer", max_length=2000)
    feedback: str | None = Field(None, description="AI feedback on the user's answer", max_length=2000)
