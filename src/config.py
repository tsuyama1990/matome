...
class SecureString:
    """A custom secure string representation for handling credentials safely."""
    ...
    @classmethod
    def validate(cls, v: typing.Union[str, "SecureString"], _info: typing.Any=None) -> "SecureString":
        if isinstance(v, SecureString):
            return v
        return cls(str(v))
    ...
    def __del__(self) -> None:
        ...
        if hasattr(self, "_value") and self._value:
            # Safely zero the memory buffer immediately upon garbage collection
            ctypes.memset((ctypes.c_char * len(self._value)).from_buffer(self._value), 0, len(self._value))
...
class CredentialConfig(MatomeConfig):
    ...
    openrouter_api_key: SecureString = Field(
        ..., description="The OpenRouter API Key. Accessed securely via EnvCredentialProvider."
    )
    openrouter_api_url: SecureString = Field(
        ...,
        description="The OpenRouter API URL.",
    )
    ssl_cert_path: SecureString | None = Field(
        default=None, description="Path to internal corporate CA cert bundle"
    )

    @field_validator("openrouter_api_key")
    @classmethod
    def validate_openrouter_api_key(cls, v: SecureString) -> SecureString:
    ...
    @classmethod
    def validate_openrouter_api_url(cls, v: SecureString) -> SecureString:
    ...
    @classmethod
    def validate_ssl_cert_path(cls, v: SecureString) -> SecureString:
